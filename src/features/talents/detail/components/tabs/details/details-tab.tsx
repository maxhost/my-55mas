...
import type { DetailsTabHints, TalentDetailContext, TalentDetailsData } from '../../../types';
import { PersonalDataSection } from './personal-data-section';
import { ContactSection } from './contact-section';
import { ProfessionalSituationSection } from './professional-situation-section';
import { TalentServicesSection } from './talent-services-section';
import { PaymentPrefsSection } from './payment-prefs-section';
import { LanguagesSection } from './languages-section';
import { OtherSurveySection } from './other-survey-section';

export type SectionKey =
  | 'personal' | 'contact' | 'professional' | 'services'
  | 'paymentPrefs' | 'languages' | 'otherSurvey';
...
const INITIAL_DIRTY: Record<SectionKey, boolean> = {
  personal: false, contact: false, professional: false, services: false,
  paymentPrefs: false, languages: false, otherSurvey: false,
};

export function DetailsTab({
  talentId, data, context, hints, locale, talentCountryId, onSectionSaved,
}: Props) {
  ...
  const common = { talentId, context, hints, locale, onSaved: onSectionSaved };
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <ProfessionalSituationSection {...common} {...wire('professional')} data={data.professional} />
      <TalentServicesSection
        {...common} {...wire('services')}
        data={data.services} talentCountryId={talentCountryId}
      />
      <PaymentPrefsSection {...common} {...wire('paymentPrefs')} data={data.paymentPrefs} />
      <LanguagesSection {...common} {...wire('languages')} data={data.languages} />
      <OtherSurveySection {...common} {...wire('otherSurvey')} data={data.survey} />
    </div>
  );
}
...
export function SectionShell({
  title, open, onToggle, editing, onStartEdit, onCancelEdit, onSave,
  saving, canEdit = true, hints, previewText, readMode, editMode,
}: SectionShellProps) {
  return (
    <Card size="sm">
      <CardHeader
...
export function Field({ label, value, fallback }: {
