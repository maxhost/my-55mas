'use server';

import { revalidatePath } from 'next/cache';
import { createClient } from '@/lib/supabase/server';
import { pinTalentNoteSchema } from '../schemas';

type Result = { data: { ok: true } } | { error: { message: string } };

export async function pinTalentNote(input: unknown): Promise<Result> {
  const supabase = createClient();

  const parsed = pinTalentNoteSchema.safeParse(input);
  if (!parsed.success) {
    return { error: { message: parsed.error.issues[0]?.message ?? 'Invalid input' } };
  }
  const { noteId, pinned } = parsed.data;

  const { error } = await supabase
    .from('talent_notes')
    .update({ pinned })
    .eq('id', noteId);
  if (error) return { error: { message: error.message } };

  revalidatePath('/[locale]/(admin)/admin/talents/[id]', 'page');
  return { data: { ok: true } };
}
