...
export function ClientsToolbar({
  ...
  return (
    <div className="flex flex-wrap items-center gap-4">
      ...
      <Select value={companyFilter} onValueChange={(val) => onCompanyFilterChange(val ?? 'all')}>
        <SelectTrigger>
          <SelectValue placeholder={t('filterByCompany')} />
        </SelectTrigger>
        <SelectContent>
          <SelectItem value="all">{t('allCompanies')}</SelectItem>
          <SelectItem value="with">{t('withCompany')}</SelectItem>
          <SelectItem value="without">{t('withoutCompany')}</SelectItem>
        </SelectContent>
      </Select>
    </div>
  );
