...
import { listFiscalIdTypes } from '@/features/service-hire/actions/list-fiscal-id-types';
...
export default async function TestServiceHireRoute({ params: { locale } }: Props) {
  ...
  const [services, fiscalIdTypes] = await Promise.all([
  ...
  return (
    <div className="p-8">
      ...
      <TestServiceHirePage services={services} locale={locale} fiscalIdTypes={fiscalIdTypes} />
    </div>
  );
}
